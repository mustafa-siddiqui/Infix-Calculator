...
Email:	msiddiq7@u.rochester.edu

Brief:	To implement this project, I wrote my own linked list class (LinkedList.java),
	and used this class to implement my own stack and queue classes (Stack.java
	and Queue.java respectively), as directed by the professor.
	...
	I read a line from the input line by using the Scanner class. The method
...
Challenges:
	I had a little trouble parsing the input string into a format that would result in
	error-free handling due to the fact that operators can be combined like '!(('.
	However, I overcame this by modifying my approach to the one right now. By inserting
	spaces between operators and operands, meaning each element will have a space around
	it. This allows for simple parsing by using the String.split() method with a space
	character as its argument.

EXTRA CREDIT:
	I believe my code qualifies for 12% extra credit. This is because the program supports
	the following:
	- ^ operator (exponent) (2%)
	- handling invalid expressions (10%)

	The sample test file which I used to test my code is 'test_input.txt'. It includes
	expressions that test the extra credit criteria as well.
...
List of files:
	...
	- test_input.txt // example test cases to test additional expressions and extra credit functionality
	- out.txt 	 // the output for 'test_input.txt'
